  ...
  return (
    <div className="flex flex-col gap-6 pb-10">
      ...
      <div className="relative group rounded-xl sm:rounded-2xl overflow-hidden shadow-2xl transition-all duration-500">
        <div className="relative bg-white/2 backdrop-blur-xl border border-white/5 rounded-xl sm:rounded-2xl overflow-hidden px-4 sm:px-6 py-5 flex flex-col gap-5">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 rounded-xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 flex items-center justify-center">
              <Globe className="w-5 h-5" />
            </div>
            <div className="flex flex-col gap-0.5">
              <h3 className="text-sm font-bold text-white tracking-wide">Add Base Domain</h3>
              <p className="text-[11px] font-medium text-white/40">This will be available as an option when creating new tunnels.</p>
            </div>
          </div>
          ...
          <div className="flex gap-2 w-full mt-2">
            <input
              ...
              className="px-4 rounded-xl text-sm outline-none transition-all h-10 border bg-white/5 text-white border-white/10 hover:border-white/20 focus:border-emerald-500/50 focus:bg-white/10 flex-1 min-w-0"
            />
            <button
          ...
          {(settings.customDomains?.length || 0) > 0 && (
            <div className="flex flex-col gap-2 mt-4 pt-4 border-t border-white/5">
              <h4 className="text-xs font-semibold text-white/50 uppercase tracking-widest mb-1">Your Base Domains</h4>
              {settings.customDomains?.map((domain) => (
                <div key={domain} className="flex flex-row items-center justify-between px-4 py-3 bg-black/20 border border-white/5 rounded-xl group hover:border-white/10 transition-colors">
                  <span className="text-sm font-medium text-white/80 select-all">{domain}</span>
                  <button
